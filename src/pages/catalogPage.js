...
class CatalogPage {
  ...
  async checkProductCatalogIsVisible() {
    await expect(this.productCatalogText).toHaveAttribute('text', 'Products');
  }
  ...
  async waitForItem(itemNumber) {
    await this.getItem(itemNumber).waitForEnabled({ timeout: 5000 });
  }
